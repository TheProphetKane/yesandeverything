---
name: code-audit
description: Comprehensive code-level audit. Catches syntax errors, parse-time hazards, recurring bug patterns (silent field guards, dual-path divergence, FUSE truncation), security violations (locked-decision breaches, secret exposure), voice violations on public artifacts, and cross-file consistency drift (version pills, constant mirrors). Project-aware, applying the per-project hazard catalogs for HBH, BR, YaB, YaC, Scheduler, YaE, and YaApothecary. Language focus is GDScript and .tscn for Godot, plus TypeScript, PowerShell, and C-family. Use whenever the user asks to audit code, review code, lint, find bugs, check for smells, do static analysis, scan for issues, run a code review, do a preship code check, "is this broken", "what could break", "scan before I ship", "deep code review". Also trigger proactively before any release.ps1 on a project not audited in 7+ days.
---
