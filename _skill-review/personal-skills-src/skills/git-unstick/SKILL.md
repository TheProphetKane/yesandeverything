...
- **Always `cd` to the repo first.** Nick works across `X:\HereBeHordes`, `X:\YesAndChains`, `X:\Scheduler`, and `X:\YesAndEverything`. Run every git command with a known cwd. If the cwd isn't already one of those four, ask which repo.
...
If cwd is already one of `X:\HereBeHordes`, `X:\YesAndChains`, `X:\Scheduler`, or `X:\YesAndEverything`, use it. Otherwise:

1. If the user named a project ("fix git on HBH"), map: HBH → `X:\HereBeHordes`, YaC → `X:\YesAndChains`, Scheduler → `X:\Scheduler`, YaE → `X:\YesAndEverything`.
