---
name: smart-commit
description: Generate a commit message that matches the project's existing style, stage the appropriate files (excluding secrets), commit, and optionally push or open a PR. Replaces the manual cycle of running git diff, eyeballing the changes, writing a message, staging, committing, pushing. Three modes - (1) commit only (analyze + stage + commit), (2) commit + push (same plus push to origin), (3) commit + push + PR (same plus open a GitHub PR via gh CLI). Also exposes a branch-hygiene mode that prunes local branches deleted on origin. Use whenever the user says "commit this", "commit these changes", "make a commit", "stage and commit", "commit and push", "smart commit", "open a PR", "send this to GitHub", "clean up merged branches", "delete gone branches", "git tidy", "branch cleanup". Honors per-project release script conventions - if the project has a scripts/release.ps1 and the change qualifies as a release (version bump intended), the skill recommends release.ps1 instead and does not commit directly.
---
