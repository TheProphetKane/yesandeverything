...
**Actionable vs deferred (load-bearing for the dashboard).** Every item the dashboard surfaces as an "open action" MUST be immediately actionable — something a session could pick up and start right now. A finding that is real but cannot be actioned yet is DEFERRED, not an open action. Treat a finding as deferred when it is any of: explicitly post-1.0 / "no pre-1.0 action"; parked in the backlog with no current step; blocked on a user decision; or gated on an external account, event, or "when X next opens" (e.g. "profile when the native build lands", "migrate off keytar before encryption goes default", "split the monolith post-1.0"). Deferred findings never count in `actionsOpen` and never go in `actions[]`; they go in `deferred[]`. A finding belongs in EITHER `actions[]` OR `deferred[]`, never both.

The JSON `barRaise.actionsOpen` field counts only the IMMEDIATELY-ACTIONABLE HIGH + MEDIUM findings. LOW and all deferred findings are excluded from that count.

The JSON `barRaise.actions[]` array carries those immediately-actionable HIGH + MEDIUM items as structured rows so the dashboard can list them inline. One object per counted action, in the ranked order of the Action items list above, each `{ "severity": "HIGH"|"MED", "title": "<the imperative action sentence, with the leading priority/severity/lens markers stripped>", "lens": "<provenance lens id>" }`. `"priority": <impact x confidence int>` is optional; `"id"` is REQUIRED, never omitted. Use a stable, deterministic id `<lens>-NN` where NN is the per-lens ordinal (`data-integrity-01`, `data-integrity-02`, ...). Dedup and closure detection track findings by id, so an id-less action is invisible to both and can never be marked closed: a project that once shipped 94 of 98 actions with no id stranded them as permanent dashboard counters. The array length equals `actionsOpen`. Keep each `title` to one sentence; the report Markdown holds the full evidence.

The JSON `barRaise.deferred[]` array carries the real-but-not-actionable-yet findings (any severity), each `{ "severity": "HIGH"|"MED"|"LOW", "title": "<one-sentence description>", "lens": "<provenance lens id>", "reason": "<why it can't be actioned now: 'post-1.0', 'blocked on Nick decision', 'needs external account', etc.>" }`. These surface only under the dashboard's separate "deferred" chip, never as open actions. Rebuild it from scratch each run so it never carries a stale row. Both arrays are additive inside the locked `barRaise` block; the dashboard ignores either when absent.
...
- `openFindings`: list of `{ id, severity, priority, runsOpen, firstSeen }` with **exactly one entry per row in `actions[]`, sharing that row's id**, so `len(openFindings)` equals `actionsOpen`. This coverage is REQUIRED and must be asserted before writing: `openFindings` is the list dedup, aging, and closure detection all read, so an open action missing from it can never be deduped, aged, or closed. This failed once with a project carrying 98 actions but only 36 openFindings, stranding 62 findings as permanent counters; if a prior run left the list short, rebuild it to cover every current action before writing. A finding carried from the previous run keeps its id and increments `runsOpen`; a new one starts at 1 with `firstSeen` set to the run date. `actionsClosed` is then computed as the count of the previous run's `openFindings` ids absent from this run's set.
...
4. Mutate ONLY the `barRaise` block. Preserve every other field. The locked fields keep their shape (`latestReportPath`, `latestReportAt`, `verdict`, `topFinding`, `actionsOpen`, `actionsClosed`); `health`, `lensScores`, `openFindings`, `tensionsOpen`, `actions`, `deferred` are additive inside the same block. Rebuild `actions` AND `deferred` from scratch each run so neither carries a stale row; `actions` length must equal `actionsOpen` (immediately-actionable HIGH+MED only), and anything real-but-not-actionable-now goes in `deferred` instead.
