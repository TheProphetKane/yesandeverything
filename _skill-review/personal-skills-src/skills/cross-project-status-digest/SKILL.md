...
description: Produce a single at-a-glance markdown digest across Nick's projects (HBH, YaC, Scheduler, YaE) covering current version, commits since last digest, active milestone, working-tree state, drift flags, and a "what to look at next" recommendation. Then chain the recommendations into work-queue-runner so they become trackable items rather than scrollback. Use whenever the user asks for a status update across projects, a weekly or daily digest, a roll-up of what changed this week, a cross-repo check-in, or phrases like "status update", "weekly digest", "what changed this week", "give me the rundown", "check in on my projects", "cross-project status", "where are things at", or "what is the state of everything". Designed to power a scheduled task and also runnable on demand. Output caps around 80 lines.
---
