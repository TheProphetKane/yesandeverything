...
- **Scheduler description** (index.html line 327-337 + `projects/scheduler/index.html`) — matches `X:\Scheduler\docs\DESIGN.md` §1, §2, §3. Aquatics department, Lifeguard + Swim Instructor sub-departments, preference-driven six-week auto-fill, review queue, mobile-first, Cloudflare Workers + D1 + Vite/React/Tailwind + Supabase magic-link. Stack and feature scope all reconcile.
- **Scheduler design.html** — password-gated (`var PASSWORD = 'SeeSchedule'`) base64 mirror of `X:\Scheduler\docs\DESIGN.md`. Same shape as hordes/index.html. Note: CLAUDE.md doesn't document this password, only `SneakPeak` for hordes/.
