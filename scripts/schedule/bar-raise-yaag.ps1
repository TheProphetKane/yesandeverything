# bar-raise-yaag.ps1 - scheduled per-project bar-raise for YesAndAgents.
#
...
# orchestrator at orchestrators/per_project.md against YesAndAgents.
#
# Output lands in:
#   - X:\YesAndAgents\docs\BAR_RAISE-YYYY-MM-DD.md
#   - X:\YesAndEverything\status\data\Agents.json (barRaise block updated)
#   - X:\YesAndEverything\scripts\schedule\logs\bar-raise-yaag-YYYY-MM-DD.log
#
# The skill itself commits + pushes the YaE-side JSON change. This shim does
# not touch git directly.
#
# Project ID is YaAg (distinct from YaA = Apothecary). Do not collapse the two.
#
...
$LogPath = Join-Path $LogDir "bar-raise-yaag-$Today.log"
...
# orchestrators/per_project.md runs against YesAndAgents.
$Prompt = "bar-raise YaAg"
...
"$(Get-Date -Format o) START bar-raise-yaag" | Tee-Object -FilePath $LogPath -Append
...
"$(Get-Date -Format o) END bar-raise-yaag exit=$rc" | Tee-Object -FilePath $LogPath -Append
