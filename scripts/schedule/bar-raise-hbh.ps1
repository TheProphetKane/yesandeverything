# bar-raise-hbh.ps1 - scheduled per-project bar-raise for Here Be Hordes.
#
...
# orchestrator at orchestrators/per_project.md against Here Be Hordes.
#
# Output lands in:
#   - X:\HereBeHordes\docs\BAR_RAISE-YYYY-MM-DD.md
#   - X:\YesAndEverything\status\data\Hordes.json (barRaise block updated)
#   - X:\YesAndEverything\scripts\schedule\logs\bar-raise-hbh-YYYY-MM-DD.log
#
...
$LogPath = Join-Path $LogDir "bar-raise-hbh-$Today.log"
...
# orchestrators/per_project.md runs against Here Be Hordes.
$Prompt = "bar-raise HBH"
...
"$(Get-Date -Format o) START bar-raise-hbh" | Tee-Object -FilePath $LogPath -Append
...
"$(Get-Date -Format o) END bar-raise-hbh exit=$rc" | Tee-Object -FilePath $LogPath -Append
