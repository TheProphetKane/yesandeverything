...
$LogPath = Join-Path $LogDir "bar-raise-scheduler-$Today.log"
...
"$(Get-Date -Format o) START bar-raise-scheduler" | Tee-Object -FilePath $LogPath -Append
...
"$(Get-Date -Format o) END bar-raise-scheduler exit=$rc" | Tee-Object -FilePath $LogPath -Append
