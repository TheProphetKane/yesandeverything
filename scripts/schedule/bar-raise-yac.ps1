...
# orchestrator at orchestrators/per_project.md against YesAndChains.
#
# Output lands in:
...
$LogPath = Join-Path $LogDir "bar-raise-yac-$Today.log"
...
# orchestrators/per_project.md runs against YesAndChains.
$Prompt = "bar-raise YaC"
...
"$(Get-Date -Format o) START bar-raise-yac" | Tee-Object -FilePath $LogPath -Append
...
"$(Get-Date -Format o) END bar-raise-yac exit=$rc" | Tee-Object -FilePath $LogPath -Append
