# bar-raise-yab.ps1 - scheduled per-project bar-raise for YesAndBudget.
#
...
# orchestrator at orchestrators/per_project.md against YesAndBudget.
#
# Output lands in:
#   - X:\YesAndBudget\docs\BAR_RAISE-YYYY-MM-DD.md
#   - X:\YesAndEverything\status\data\Budget.json (barRaise block updated)
#   - X:\YesAndEverything\scripts\schedule\logs\bar-raise-yab-YYYY-MM-DD.log
#
...
$LogPath = Join-Path $LogDir "bar-raise-yab-$Today.log"
...
# orchestrators/per_project.md runs against YesAndBudget.
$Prompt = "bar-raise YaB"
...
"$(Get-Date -Format o) START bar-raise-yab" | Tee-Object -FilePath $LogPath -Append
...
"$(Get-Date -Format o) END bar-raise-yab exit=$rc" | Tee-Object -FilePath $LogPath -Append
