...
Action index (additive, 2026-06-17): the bar-raise writes `barRaise.actions[]`, one object per counted HIGH/MED action item in ranked order (`{severity, title, lens}`, optional `priority` / `id`). Length equals `actionsOpen`; LOW is excluded. The dashboard lists these inline under the open-actions count so the actions tab shows the real items, not just the number. `scripts/backfill_barraise_actions.py` retrofits the array into existing status JSON from each project's latest `BAR_RAISE-*.md` and is re-runnable; the writer rebuilds the array from scratch every run.
