...
You are working on **YesAndEverything** — the public-facing static site at <https://yesandeverything.com>. It is a single-page landing page listing Kane's projects, plus per-project sub-pages, plus a password-gated mirror of the HTBH GDD.
...
- **Is not:** the actual code of the projects it links to. Each project (HTBH, Chains, Scheduler) lives in its own repo. This repo only carries landing pages + mirrors.
...
| `PERSONAL_CLAUDE_ARCHITECTURE.md` | The handler-and-canonical pattern spec applied to all four personal projects. Companion to CLAUDE_SETTINGS. |
...
For HTBH GDD republishing, do **not** edit this repo directly — run `X:\HereThereBeHordes\scripts\publish-gdd.ps1` and it'll push the injection here for you.
...
2. For the `hordes/` mirror, work flows from the HTBH repo, never the other way.
3. For per-project page content, mirror what the project's own canonical doc says (GDD for HTBH, DESIGN.md for Scheduler). Don't fabricate.
